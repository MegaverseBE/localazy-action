...
DOWNLOAD_COMMAND="localazy download"
UPLOAD_COMMAND="localazy upload"

if [ ! -z "$INPUT_CONFIG_FILE" ]; then
  DOWNLOAD_COMMAND="$UPLOAD_COMMAND -c $INPUT_CONFIG_FILE"
  UPLOAD_COMMAND="$UPLOAD_COMMAND -c $INPUT_CONFIG_FILE"
fi

if [ ! -z "$INPUT_READ_KEY" ]; then
  DOWNLOAD_COMMAND="$UPLOAD_COMMAND -r $INPUT_READ_KEY"
  UPLOAD_COMMAND="$UPLOAD_COMMAND -r $INPUT_READ_KEY"
fi

if [ ! -z "$INPUT_WRITE_KEY" ]; then
  DOWNLOAD_COMMAND="$UPLOAD_COMMAND -w $INPUT_WRITE_KEY"
